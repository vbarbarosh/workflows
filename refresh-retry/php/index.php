...
{
    $rrule = RRule::createFromRfcString($params['rrule']); // "DTSTART:20250101T000000\nRRULE:FREQ=HOURLY;INTERVAL=2"
    $timeout = new DateInterval($params['timeout']); // PT30M
    $retry_delay = $params['retry_delay'] ?? []; // [null, 'PT0M', 'PT1M', 'PT5M', 'PT15M', 'PT30M', 'PT1H']
    $attempt_no = $params['attempt_no'] ?? 0;

    switch ($params['action']) {
    case 'start':
        $deadline_at = now()->add($timeout);
        call_user_func($params['fn'], [
            'refresh_at' => $rrule->getNthOccurrenceAfter($deadline_at, 1),
    ...
    case 'success':
        call_user_func($params['fn'], [
            'refresh_at' => $rrule->getNthOccurrenceAfter(now(), 1),
            'deadline_at' => null,
            'attempt_no' => 0,
            'final_failure' => false,
        ]);
